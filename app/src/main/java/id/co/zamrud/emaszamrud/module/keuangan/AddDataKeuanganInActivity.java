...
public class AddDataKeuanganInActivity extends AppCompatActivity implements ButtonCallBack {
    ...
    public void callbackButtonReturn(FormElementButton formElementButton, Object o) {
        ...
        if (validatorResult.size() <= 0) {
            final DataKeuanganItemClass dataPemasukan = new DataKeuanganItemClass();
            dataPemasukan.setJenis(formElementJenisPemasukan.getValue());
            dataPemasukan.setJumlah(formElementJumlahPemasukan.getValue());
            dataPemasukan.setTanggal(mTanggal);
    ...
    private void startSave(DataKeuanganItemClass dataPemasukan) throws JSONException {
        final String[] message = new String[2];

        ObjectMapper mapper = new ObjectMapper();
        try {
            String ajsonparam = mapper.writeValueAsString(dataPemasukan);
            IDatabaseClient databaseClient = ApiGenerator.createService(IDatabaseClient.class);
            final Call<PostExecuteDatabase> call = databaseClient.insertDataPemasukan(ajsonparam);
